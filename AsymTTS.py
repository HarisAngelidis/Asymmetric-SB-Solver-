...
CONFIG = {
    ...
    "B": 100, "steps": 300, "seeds": None,             # batch size, annealing steps
    "dt": 0.1, "a0": 1.0, "noise": 0.05,               # time step, pump target a0, symmetry-breaking noise
    ...
    #   P_min/P_max/P_exp = penalty weight ramp 
    #   init_amp = amplitude of random init
}
...
def asymmetric(A_sp, N, cfg, seed):
    # Asymmetric Simulated Bifurcation for MIS, run as B parallel trajectories.
    # Only ACTIVE units (x>0) exert repulsion.
    #symmetry-breaking noise escapes the empty set.
...
def run(cfg):
    ...
    for inst in cfg["instances"]:
        if not os.path.exists(inst["path"]):                               # skip missing files
...
def measure_tts(path, target, B=2000, seeds=None, p_t=0.99, cfg=None):
    ...
    if seeds is None: seeds = make_seeds()                                # fresh seeds for this run
    if not os.path.exists(path):
        print(f" file '{path}' not found"); return None
    A = load_dimacs(path); N = A.shape[0]; A_sp = to_sp(A)                # load graph in sparse form
    _ = asymmetric(A_sp, N, dict(cfg, B=8), 999)                          # warm-up run
    ...
    if succ == 0:                                                          # target never reached -> TTS undefined
        ...
        return float("inf")
    N_traj = 1.0 if p >= 1.0 else np.log(1-p_t)/np.log(1-p)                # trajectories for prob p_t
    TTS = N_traj * (T_batch / B)                                           # time-to-solution
...
if __name__ == "__main__":
    # 1) score table (best IS + success%) 
    run(CONFIG)

    #2) TTS 
    measure_tts("p_hat1500-3.clq.txt", target=11, B=2000)      # seeds=None -> 10 random seeds

    #3) print a solution for validation
